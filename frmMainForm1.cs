            ...
            else
            {
                if (active)
                {
                    ...
                    foreach (string pdfFile in pdfFileList) {
                        ...
                        HFP.PrintPdf(HFP.GetConfig("selectedPrinter"), pdfFile, Path.GetFileNameWithoutExtension(pdfFile));
                        // 3. Move pdf files to processed folder
                        HFP.LogWriteLine("Moving pdf file to processed folder: " + pdfFile);
                        File.Move(pdfFile, Path.Combine(HFP.GetConfig("processedDir"), Path.GetFileName(pdfFile)));
                    }
                }
            }
        }
